...
async function prepareFileForTyping(filename: string): Promise<void> {
	if (!director || !workingDirectory) {
		return;
	}

	if (!director.shouldRewriteFile(filename)) {
		return;
	}
	...
	try {
		await vscode.workspace.fs.delete(fileUri, { useTrash: false });
		createdFiles.delete(filename);
		log(`Rewriting file for extension: ${filename}`);
	} catch (error) {
		log(`Failed to delete file for rewrite ${filename}: ${error}`);
	} finally {
		director.clearRewriteFlag(filename);
	}
}
...
async function handleNextFile(currentEditor: vscode.TextEditor): Promise<void> {
	...
	await prepareFileForTyping(nextFile.filename);
...
async function handleRandomFileSwitch(currentEditor: vscode.TextEditor): Promise<void> {
	...
	await prepareFileForTyping(newFile.filename);
